...
import { SearchUserById, SearchUserImage, UpdateUserAction, UpdateUserActionImage, } from "@/action/auth-action";
...
const SingleLesson =   ( {params} : { params:{ UserAddress: string ; } }) => {
  const { data: session, status } = useSession();
  const id = parseInt(params.UserAddress as string)
  ...
  const idnumber =  parseInt(params.UserAddress);
  const [formData,setFormData] = useState<{  id:number;  name :String ;  family:String ;   expertise:String ;   education:String ;  phon :string; email:string , password:string }>({id:0 , name:'ناشناخته' , email:'ناشناخته' , family:'ناشناخته', password:'ناشناخته',education:'ناشناخته',expertise:'ناشناخته',phon:'' });
...
useEffect(() => {
  const fetchData2 = async () => {
      try {
        const user3 =await SearchUserById(idnumber)
        setFormData(user3[0] as any)
 ...
 useEffect(() => {
  const fetchData = async () => {
      try {
       const userImage  = await SearchUserImage(formData.email as string);
       const erfan = new Uint8Array(userImage as any) 
       const blob =  new Blob([erfan], {type: 'image/*'})       
...
useEffect(() => {
  const fetchData11 = async () => {
    ...
    formData3.append('email', formData.email); // تغییر نام به image
     const res2 = await UpdateUserActionImage(formData3 as any);
   ...
   <div className="mx-auto max-w-242.5">
      ...
      <EditIcon onClick={handleOpen}/>
          ...
          <div className="px-4 pb-6 text-center lg:pb-8 xl:pb-11.5">
            <div className=" mb-20 relative z-30 mx-auto -mt-22 h-30 w-full max-w-30 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:max-w-44 sm:p-3">
            ...
            <div className="mt-4">
              ...
              <div className="mx-auto mb-5.5 mt-4.5 grid max-w-125 grid-cols-3 rounded-md border border-stroke py-2.5 shadow-1 dark:border-strokedark dark:bg-[#37404F]">
                ...
                <div className="flex flex-col items-center justify-center gap-1 border-r border-stroke px-4 dark:border-strokedark xsm:flex-row">
                  ...
                  <span className="text-sm">تحصیلات</span>
                </div>
